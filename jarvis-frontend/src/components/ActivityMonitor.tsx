...
const activityDetails: Record<AssistantState, { label: string; entries: string[] }> = {
  ...
  thinking: { label: "PROCESSING", entries: ["REQUEST SENT", "I AM THINKING", "KEEPING CONTEXT SHORT"] },
...
export function ActivityMonitor({ state }: ActivityMonitorProps) {
  ...
  return (
    <aside className={`activity-monitor state-${state}`} aria-label="AM activity monitor">
